...
# Frame per second untuk mengatur kecepatan loop permainan
FPS            = 60
# Direktori file ini, lalu folder asset untuk gambar dan suara
BASE_DIR       = os.path.dirname(os.path.abspath(__file__))
ASSET_DIR      = os.path.join(BASE_DIR, "assets")

# Ukuran target untuk gambar pemain, ikan, dan bom
PLAYER_TARGET_W = 110
FISH_TARGET_W  = 84
BOMB_TARGET_W   = 84
...
# Warna umum yang digunakan di HUD dan teks
C_WHITE  = (255, 255, 255)
C_BLACK  = (0,   0,   0)

# Kecepatan dasar dan peningkatan untuk player, ikan, dan bomb
PLAYER_SPEED        = 10
FISH_BASE_SPEED    = 4
FISH_MAX_SPEED     = 12
FISH_SPEED_STEP    = 1.2
...
# Batasan jumlah miss sebelum game over
MAX_MISS            = 3

# Shrink untuk hitbox agar collision lebih adil dan tidak terlalu ketat
...
class GameObject:
    # GameObject adalah kelas dasar untuk semua objek yang muncul di layar.
    # Semua objek memiliki gambar, posisi, dan ukuran.
    ...
    def get_rect(self) -> pygame.Rect:
        # Membuat kotak (rect) untuk collision detection.
        return pygame.Rect(self.x, self.y, self.width, self.height)

    def draw(self, surface: pygame.Surface):
        # Gambar object di layar pada posisi x, y.
        surface.blit(self.image, (self.x, self.y))


class Player(GameObject):
    # Player adalah karakter utama yang bisa digerakkan oleh pemain.
    ...
    def move(self, keys):
        # Gerakkan player kiri / kanan berdasarkan input.
    ...
    def trigger_eat_animation(self, duration_frames: int = PLAYER_EAT_ANIM_FRAMES):
        # Aktifkan animasi makan untuk beberapa frame.
        self.eat_anim_timer = max(1, duration_frames)
        self.image = self.eat_image

    def update_animation(self):
        # Kurangi timer animasi setiap frame.
...
class Fish(GameObject):
    # Fish mewarisi GameObject, jadi ia sudah punya gambar dan posisi.
    ...
    def update(self):
        # Ikan jatuh ke bawah dan berputar sedikit.
        self.y += self.speed
        self.angle = (self.angle + self.rotate_speed) % 360

    def is_off_screen(self) -> bool:
        # Cek apakah ikan sudah melewati bagian bawah layar.
        return self.y > SCREEN_H

    def draw(self, surface: pygame.Surface):
        # Gambar ikan yang diputar agar terlihat hidup.
...
class Bomb(GameObject):
    ...
    def update(self):
        # Bom bergerak turun lurus ke bawah.
        self.y += self.speed

    def is_off_screen(self) -> bool:
        # Cek apakah bom sudah melewati layar.
        return self.y > SCREEN_H
...
def load_image_scaled(name: str, target_w: int) -> pygame.Surface:
    # Muat gambar dari folder assets dan ubah skalanya.
...
def load_fish_images(target_w: int) -> list[pygame.Surface]:
    # Cari semua file gambar ikan di folder assets.
    fish_names = sorted(
        file_name
    ...
    # Jika tidak ada daftar ikan, pakai fish.png default.
    if not fish_images:
...
def load_heart_image(name: str, target_w: int):
    ...
    # Muat gambar hati dan pangkas area transparan di sekitarnya.
...
def load_ground_image(name: str, target_h: int):
    ...
    # Muat gambar tanah (ground) dan pangkas area transparan.
    img = pygame.image.load(path).convert_alpha()
    bbox = img.get_bounding_rect(min_alpha=1)
    if bbox.width > 0 and bbox.height > 0:
        img = img.subsurface(bbox).copy()
...
def screen_start(surface, clock, player_img, fish_imgs, bomb_img,
    ...
    # Layar awal: menunggu pemain menekan ENTER untuk mulai.
    tick = 0
    while True:
        tick += 1  # Hitung frame untuk animasi.
        draw_background(surface, ground_img, sky_img)  # Gambar latar belakang.

        title = font_big.render("Catch the Fish!", True, (200, 50, 30))
        surface.blit(title, (SCREEN_W // 2 - title.get_width() // 2, 75))
        bob = int(math.sin(tick * 0.05) * 8)  # Hitung gerakan naik turun demo.
        fish_preview = fish_imgs[(tick // 25) % len(fish_imgs)]
        surface.blit(fish_preview,  (60,  175 + bob))  # Gambar ikan demo.
        surface.blit(player_img, (SCREEN_W // 2 - player_img.get_width() // 2, 160))
        surface.blit(bomb_img,   (SCREEN_W - 150, 175 + bob))  # Gambar bom demo.
        ...
        for i, (txt, col) in enumerate(lines):
            r = font_sm.render(txt, True, col)  # Buat teks instruksi.
            surface.blit(r, (SCREEN_W // 2 - r.get_width() // 2, 308 + i * 34))

        if (tick // 30) % 2 == 0:
            s = font_med.render("[ ENTER ]  Mulai", True, (40, 40, 180))
            surface.blit(s, (SCREEN_W // 2 - s.get_width() // 2, 465))  # Tekan ENTER ?

        pygame.display.flip()  # Perbarui layar.
        clock.tick(FPS)  # Batasi frame rate.

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit(); sys.exit()  # Keluar bila klik close.
            if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                return  # Keluar dari layar start dan mulai game.
...
def screen_game_over(surface, clock, score: int, reason: str,
    ...
    # Layar akhir: tampilkan skor dan pilihan kembali/keluar.
    tick = 0
    while True:
        tick += 1
        draw_background(surface, ground_img, sky_img)  # Gambar latar belakang.
        ...
        ov.fill((0, 0, 0, 145))  # Buat overlay gelap agar teks terlihat.
        surface.blit(ov, (0, 0))
        ...
        if (tick // 30) % 2 == 0:
            for txt, col, y in [
                ...
                r = font_sm.render(txt, True, col)  # Buat teks tombol.
                surface.blit(r, (SCREEN_W // 2 - r.get_width() // 2, y))

        pygame.display.flip()  # Tampilkan semua yang sudah digambar.
        clock.tick(FPS)  # Batasi frame rate.

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit(); sys.exit()  # Tutup program bila jendela ditutup.
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN: return True  # Main lagi.
                if event.key == pygame.K_ESCAPE: return False  # Keluar.
...
def run_game(surface, clock, player_img, player_eat_img, fish_imgs, bomb_img,
    ...
    # Fungsi utama yang menjalankan loop permainan hingga game over.
    player = Player(player_img, player_eat_img)  # Buat objek player di layar.
    fishes: list = []                           # List untuk menampung ikan yang muncul.
    bombs: list = []                            # List untuk menampung bom yang muncul.

    spawn_timer         = 0  # Timer untuk spawn objek.
    spawn_interval      = SPAWN_BASE_INTERVAL
    spawn_speedup_timer = 0  # Timer untuk mempercepat interval spawn.
    fish_speedup_timer = 0  # Timer untuk mempercepat kecepatan ikan.
    bomb_speedup_timer  = 0  # Timer untuk mempercepat kecepatan bom.
    fish_speed         = FISH_BASE_SPEED
    bomb_speed          = BOMB_BASE_SPEED
    game_over_reason = ""  # Pesan yang ditampilkan saat game over.
    running        = True

    while running:
        clock.tick(FPS)  # Jaga agar loop berjalan pada kecepatan fixed FPS.
        ...
        spawn_timer += 1  # Tambah counter spawn setiap frame.
        if spawn_timer >= spawn_interval:
            spawn_timer = 0

            # Tentukan apakah yang muncul adalah bom atau ikan.
        ...
        keys = pygame.key.get_pressed()  # Baca input tombol.
        player.move(keys)  # Gerakkan player sesuai tombol.
        player.update_animation()  # Update animasi player.

        p_rect = shrink_rect(
            player.get_rect(),
            PLAYER_HITBOX_SHRINK_X,
            PLAYER_HITBOX_SHRINK_Y
        )  # Hitbox player disusutkan untuk collision yang lebih adil.

        for fish in fishes[:]:
            fish.update()  # Perbarui posisi ikan.

            catch_zone = pygame.Rect(
                player.x - 6, player.y,
                player.width + 12, player.height // 2
            )  # Zona di depan player untuk menangkap ikan.

            fish_hitbox = shrink_rect(
                fish.get_rect(),
                FISH_HITBOX_SHRINK_X,
                FISH_HITBOX_SHRINK_Y
            )  # Hitbox ikan disusutkan.
            if fish.active and catch_zone.colliderect(fish_hitbox):
                player.score += 1  # Tambah skor saat ikan tertangkap.
                player.trigger_eat_animation()  # Mainkan animasi makan.
                if eat_sound is not None:
                    eat_sound.play()  # Putar suara ketika menangkap.
                fish.active = False
                fishes.remove(fish)
                continue

            if fish.is_off_screen():
                player.miss += 1  # Tambah miss jika ikan lepas.
                fishes.remove(fish)
        ...
        for bomb in bombs[:]:
            ...
            if bomb.is_off_screen():
                bombs.remove(bomb)  # Hapus bom yang sudah keluar layar.

        draw_background(surface, ground_img, sky_img)  # Gambar latar belakang.

        for fish in fishes:
            fish.draw(surface)  # Gambar semua ikan.

        for bomb in bombs:
            bomb.draw(surface)  # Gambar semua bom.

        player.draw(surface)  # Gambar player.

        draw_hud(surface, player, heart_img, font_med, font_sm)  # Gambar HUD.

        pygame.display.flip()  # Perbarui layar dengan semua gambar.
...
def main():
    ...
    screen = pygame.display.set_mode((SCREEN_W, SCREEN_H))  # Buat jendela game.
    pygame.display.set_caption("Catch the FISH")  # Atur judul jendela.
    clock = pygame.time.Clock()  # Buat clock untuk kontrol FPS.

    # Muat semua aset yang diperlukan sebelum permainan dimulai.
    ...
    # Siapkan font untuk teks di layar.
    ...
    high_score = 0  # Simpan skor tertinggi selama sesi.

    # Tampilkan layar awal sampai pemain menekan ENTER.
    screen_start(screen, clock, player_img, fish_imgs, bomb_img,
                 ground_img, sky_img, font_big, font_med, font_sm)

    while True:
        # Jalankan sesi permainan dan dapatkan skor akhir.
        ...
        is_new_high = score > high_score  # Cek apakah skor baru lebih tinggi.
        if is_new_high:
            high_score = score  # Simpan skor tertinggi baru.
        play_again = screen_game_over(screen, clock, score, reason,
                                      high_score, is_new_high,
                                      ground_img, sky_img, font_big, font_med, font_sm)
        if not play_again:
            break  # Hentikan permainan jika pemain memilih keluar.

    pygame.quit()  # Tutup modul pygame.
